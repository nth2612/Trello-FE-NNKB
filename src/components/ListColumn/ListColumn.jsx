...
const ListColumn = memo(function ListColumn({ boardBarHeight, orderedColumns, createNewColumn, createNewCard }) {
      ...
      <Box sx={{ mt: '12px', flexGrow: 1 }} >
        <Box sx={{ height: '100%' }} >
          <Box sx={{ padding: '2px 6px 8px', height: '100%', display: 'flex', flexDirection: 'row' }} >
            {/* Column */}
            {orderedColumns.map(col => <Column key={col._id} column={col} boardBarHeight={boardBarHeight} createNewCard={createNewCard} />)}
